...
% Rhino poaching has escalated in recent years due the demand for rhino horn in Asian countries. Rhino horn is used in traditional Chinese medicine and as a status symbol of success and wealth.

The demand for rhino horn continues to grow, and thus illegal rhino poaching. 
Wild life managers attempt to secure
rhinos with approaches such as devaluing the horn. 
...
The approach described in this paper builds previous and investigates the 
interactions between the poachers using evolutionary game theory. 
Using evolutionary game theory, this paper determines which strategy is preferred
by the poacher in various different populations of poachers.
...
Additionally, poachers behaving indiscriminately is stable and 
...
% TODO Modify this with the study of the new utility
...
(poison and dyes fade or GPS trackers can be removed). Moreover, several methods
include addressing the problem by reducing the demand of the market. In~\cite{Duan2013} the authors
purpose meeting the demand for rhino horn through a legal market by farming the
rhino horn from live rhinos. In comparison, \cite{Harvey2017} considered ivory
and stated that by enforcing a domestic ivory trade ban we can reduce the 
market's demand. This paper builds on the work of
...
always choose to behave indiscriminately, and thus the game settles to the bottom
right quadrant, i.e., the poachers win.
...
of these strategies. In a population full of indiscriminate poachers would there be 
...
indiscriminately to have any source of utility. Theorem~\ref{theorem:selective_new_utility} states that a  mixed population can be stable
with a disincentive, as opposed to Theorem~\ref{theorem:mixed} which states 
that a mixed stable strategy does not exist for \(0 <r <1\). This is illustrated in 
Fig.~\ref{fig:ess-new-utility} where the evolutionary dynamics are represented 
...
%. This is evident in Fig.~\ref{fig:convergence-over-r}, 
where
a difference in the value from \(r=0.7\) to \(r=0.5\) changes the point of convergence from a mixed to
a selective one. Moreover, the manager can also manipulate how fast
the population convergences, as shown in the difference from \(r=0.5\) to
\(r=0.2\).
Fig~\ref{fig:equilibrium-over-r-tilde-u} 
shows the  equilibrium for \(x\) for a number of different values of \(r\), 
and indicates that having a large \(r\) pushes poachers to behave indiscriminately. However all selective poachers can save at most, the $r$ proportion of dehorned rhinos, so a low $r$ is not ideal.
As demonstrated in Fig~\ref{fig:equilibrium-over-H-tilde-u}, a higher value of
\(H\) also has a non-ideal effect on the poachers, as one would expect. Thus, high values of \(H\) and 
\(r\) lead to more indiscriminate poachers.
...
Dehorning wild rhinos was introduced in 1989 as a means of securing the
safety of the animal. Our results question the effectiveness of this key strategy 
used by wild life managers to deter poachers for more than 20 years, as well as 
other devaluing techniques such as GPS tracking, poison and dyes. 
Devaluing can be effective if and only if poachers chose to behave in a 
selective manner. By exploring the dynamics of a selective population it was 
proven than being selective is not a best response even in a population where 
everyone is behaving selectively.
...
a partial horn will increase by removing horns for the world thus the probability 
of being selective is decreasing. Therefore, even in the scenario with a 
disincentive, the proportion of rhinos which could be saved 
is limited. An approach similar to the works of~\cite{Duan2013,Harvey2017} could be 
applied to address the demand of the market.
...
key to fight the illegal trade of wildlife. Strengthening disincentives for
illegal behaviour can be interpreted as the disincentive factor. Likewise,
increasing incentintives for wildlife stewardship, decreasing the cost of living
with wildlife and supporting livelihood that is not related to poaching can 
serve as incentives for selective behaviour.

It was displayed that behaving indiscriminate is more profitable than behaving 
selectively. Thus, it is an easy decision for the poachers. We suggest that a
new strategy has to be introduced to the game, a strategy that would support
new livelihoods that are not related to poaching. Another third strategy that can 
be consider is the strategy to exit the game. If a poacher believes that the cost
of poaching is too high, either because of the risk or the seeking cost, the poacher would chose to
leave the area. A further piece of work could investigate a new game where
poachers are given more than two strategies.
...
such that a population of selective poachers is now feasible.
