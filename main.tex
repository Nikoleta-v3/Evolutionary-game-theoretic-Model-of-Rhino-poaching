...
% \author{Nikoleta E. Glynatsi$^{1,\ast}$ \\ 
% Vincent Knight$^{1}$ \\ 
% Tamsin E. Lee$^{2}$}
...
% \noindent{} 1. Cardiff University, School of Mathematics, UK;

% \noindent{} 2. Cardiff University, School of Mathematics, UK;

% \noindent{} 3. Not affiliated.

% \noindent{} $\ast$ Corresponding author; e-mail: glynatsine@cardiff.ac.uk.

% \bigskip

\textit{Keywords}: Game theory, Evolutionary dynamics, Rhinoceros, 
Poachers' interactions, Wildlife.

\bigskip
\textit{Number of words}: 3624.
...
Rhino populations are at a critical level due to rhino poaching.
Wild life managers attempt to secure rhinos with approaches such as devaluing the
horn. The most common strategy of devaluation is dehorning. Game theory has been
used before to examine the interaction of poachers and managers. A manager can
either `dehorn' their rhinos or
...
work and investigates the interactions between poachers. Using evolutionary
game theory, we determine which strategy is preferred by a poacher in various
different populations of poachers. The purpose of this work is to discover whether
conditions which encourage poachers to behave selectively exist, that is,
they only kill full horned rhinos. Notwithstanding, the analytical
...
is robust. The model is adapted further to include a
...
worldwide~\citep{Amin2006}.
The illegal trade in rhino horn supports aggressive poaching syndicates and a
black market~\citep{Nowell1992, Warchol2003}. This lucrative market entices people to invest
...
level resulting in concerns over their future existence~\citep{Duan2013,Smith1993}.
...
include addressing the problem by reducing the demand in the market. In~\citep{Duan2013}
...
recently by \citep{Taylor2017}. Moreover, the debate is not limited to rhinos -
\citep{Harvey2017} considered ivory and stated that by enforcing a domestic ivory
trade ban we can reduce the market's demand. This paper builds on the work of
\citep{Lee} and considers the general strategy of devaluing horns, which includes
dehorning.

Rhino populations now persist largely in protected areas or on private land, and
require intensive protection~\citep{Ferreira2014}. For wildlife managers law 
...
\citep{Milner1992}. In~\citep{Milner1992} they found the 
...
The work of~\citep{Lee} did not take in to account the population dynamic effect
...
is explored here using evolutionary game theory~\citep{Smith}. The
...
In Section ``The Utility Model'', we determine expressions
...
As discussed briefly in Section ``Introduction'', a rhino poacher
...
In Section ``Evolutionary Stability'', the notions of stability
...
Using the utility model described in Section ``The Utility Model'',
...
This implies that whilst~\citep{Lee} identified the individual poachers
...
Using the utility model described in Section ``The Utility Model'', a population
...
game identified in~\citep{Lee} but also is a very robust and an attractive
...
Using the utility model described in Section ``The Utility Model'', a mixed
...
has been archived at~\citep{Glynatsi2017}.
...
This is done using numerical integration implemented  in~\citep{scipy}.
...
The insights gained from the results are discussed in Section ``Discussion and
Conclusions''.
...
As such approaches which aim to reduce demand (as suggested in~\citep{Duan2013})
