...
Rhino populations are at a critical level due to the demand for rhino 
horn and the subsequent poaching. Wild life managers attempt to secure
...
model this will be represented by \(\chi=(x_1, x_2)\) with \(x_1\) proportion of
the population using a strategy of the first type and \(x_2\) of the second. We
...
a poacher would never adopt a selective strategy.
...
disincentive, the proportion of rhinos which could be saved is limited. An approach
similar to the works of~\cite{Duan2013,Harvey2017} could be applied to address
...
in the model. However, standard practice in almost all cases is to either dehorn
all the animals in a defined enclosed area, or none at all. This is because partial
dehorning tends to cause havoc in rhino social structures. % Couldn't find any reference.
Our theoretical model does not apply as a representation of a single wildlife park
but instead it applies to a set of wildlife parks, countries or even continents.
There the proportion of dehorned rhinos \(r\) is the proportion of the overall population
of the animal being dehorned when each park can chose between behorning all or 
none.

Fig.~\ref{fig:equilibrium-over-r-tilde-u} can be interpreted as a situation where
2 wildlife parks exist in the world. At the point \(r=0.8\), one park that
protects 0.8 population of rhinos has chosen to dehorn and the second park did not.
A more detailed comparison between nearby properties that behave differently
could provide interesting insights, but we believe that it should be a research 
work of its own.

Empirically dehorning performs best in protecting rhinos from poaching, we have
been informed of this after discussing with an environmental conservation
economist. The model states that partially dehorning in the world or applying
more security will not have a strong effect to the poachers' behaviour,
indiscriminate dehaviour is still dominant. In a world though that all horn 
has been removed even a indiscriminate poacher will have no profit. To further
explore this scenario we suggest that a new strategy could be introduced, which
would correspond to a third option of not poaching this particular environment.
This could correspond to not poaching at all (perhaps through better opportunities)
or choosing to leave the current environment and poach elsewhere.
