...
\begin{figure}[!htbp]
    ...
    \alpha=2, \beta=.99, t=1.5, \theta_r=0.01\).}
\end{figure}
...
In Section~\ref{section:evolutionary_stability}, these two observations will be
...
+ 1\right)^{\alpha + 1}} \left(J_1 - J_2 \right) \end{equation}
...
\begin{equation}
    ...
    \frac{F + H \theta_{r} (1 - r) ^{1- \alpha -\beta}}{F}
\end{equation}
\end{theorem}
...
Direct substitution gives:
    ...
    \left(F \left(- r + 1\right)^{\beta} \left(- r + 1\right)^{\alpha + 1}
    \left(r t - r - r^{\lceil{\frac{1}{\theta_{r}}}\rceil} t\right) - H r \theta_{r} \left(r - 1\right)^{2}\right)\\
		&= \left(F
\left(1 - r\right)^{\beta - 1} \left(r - t(r -
r^{\lceil{\frac{1}{\theta_{r}}}\rceil} )\right) + H r \theta_{r} \left(1 -
r\right)^{-\alpha}\right)
    \end{align}
...
\begin{align}
    \left(F \left(1 - r\right)^{\beta - 1}r+ H r \theta_{r} \left(1 -
    r\right)^{-\alpha}\right) & < F
    \left(1 - r\right)^{\beta - 1} t(r -
    r^{\lceil{\frac{1}{\theta_{r}}}\rceil} )\\
    \frac{r}{r - r^{\lceil{\frac{1}{\theta_{r}}}\rceil}}
    \frac{F
+ H \theta_{r} \left(1 -
r\right)^{1-\beta-\alpha}}{F} & < t
\end{align}
...
\begin{itemize}
    \item Note that limit of rhs as \(r\to1^-\) is \(\infty\) so simply dehorning
        all rhinos is not a valid approach.
    \item The equilibirum predicted in~\cite{Lee} can be obtained in specific
        setting.
\end{itemize}
...
\begin{equation}
    t < \frac{1}{1 - r^{\lceil{\frac{1}{\theta_{r}}}\rceil - 1}} \frac{F + H
    \theta_{r}(1 - r)^{1-\beta}\left(r \theta_{r} - r + 1\right)^{-\alpha}}{F   }
\end{equation}
\end{theorem}
...
Direct substitution gives:
    ...
    \begin{align}
        J(0) =& \frac{1}
                     {(r-1)(r\theta_r -r + 1)^{\alpha + 1}}
                 \left(F(1-r)^{\beta}
                       (r\theta_r - r + 1)^{\alpha + 1}
                       \left(r-rt+r^{\lceil\frac{1}{\theta_r}\rceil}t\right)
                       -
                       Hr\theta_r(r-1)(-r\theta_r + r - 1)\right)\\
        J(0) =& \left(F(1-r)^{\beta-1}
                       \left(r+t(r^{\lceil\frac{1}{\theta_r}\rceil}-r)\right)
                       +
                       Hr\theta_r(r\theta_r - r + 1)^{-\alpha}\right)\\
\end{align}

The required condition is \(J(0)<0\):

\begin{align}
    \left(F(1-r)^{\beta-1}r
       +
   Hr\theta_r(r\theta_r - r + 1)^{-\alpha}\right) & <
                    F(1-r)^{\beta-1} t(r - r^{\lceil\frac{1}{\theta_r}\rceil})\\
   \frac{r}{r - r^{\lceil\frac{1}{\theta_r}\rceil}}
   \frac{F + H(1-r)^{1-\beta}\theta_r(r\theta_r - r + 1)^{-\alpha}}
        {F}
        & < t
\end{align}
...
\begin{itemize}
    \item Note that limit of rhs as \(r\to1^-\) is \(\infty\) so simply dehorning
        all rhinos will in fact imply that all poachers become indiscriminate.
    \item The equilibirum predicted in~\cite{Lee} can be obtained in specific
        setting.
\end{itemize}
...
stable population (\(s=s^*\)) exists if and only the following two conditions
hold:

\begin{equation}
    \frac{Hr\theta_r(1-r)^{1-\beta} + rF}
         {F\left(r - r^{\lceil\frac{1}{\theta_r}\rceil}\right)}
    \leq t
    \label{eqn:real_valued_constraint}
\end{equation}

\begin{equation}
    (r\theta_r+1-r)^{\alpha} >
        \frac{-Hr\theta_r(1 - r)^{1-\beta}}
        {F\left(-rt+r+r^{\lceil\frac{1}{\theta_r}\rceil}t\right)}
    > (1-r)^{\alpha}
    \label{eqn:probability_constraint}
\end{equation}

If (\ref{eqn:real_valued_constraint}) and (\ref{eqn:probability_constraint})
holds then \(x^*\) is given by:

\begin{equation}
    x^* = \frac{1}{r\theta_r}\left(
        r\theta_r - r - \left(
                    \frac{-Hr\theta_r(1 - r)^{1-\beta}}
                    {F\left(-rt+r+r^{\lceil\frac{1}{\theta_r}\rceil}t\right)}
                        \right)^{\frac{1}{\alpha}} + 1
    \right)
    \label{eqn:mixed_equilibria}
\end{equation}

\end{theorem}

\begin{proof}
    Equation (\ref{eqn:mixed_equilibria}) follows from equating (\ref{eqn:USchi}) and
    (\ref{eqn:UnotSchi}). Ensuring \(x^*\in\mathbb{R}\) gives
    (\ref{eqn:real_valued_constraint}) and ensuring \(0 < x < 1\) gives
    (\ref{eqn:probability_constraint}).
\end{proof}
...
a population of selective poachers is evolutionarily stable if and only if:

\begin{equation}
    t > \frac{1}{1 - r^{\lceil{\frac{1}{\theta_{r}}}\rceil - 1}}
    \frac{F + H \theta_{r} (1 - r) ^{1- \alpha -\beta}}{F} - \Gamma
\end{equation}

\end{theorem}

\begin{proof}
    The proof follows by calculating the modified linear approximation
    condition:

    \[
        J_{\Gamma}(x) = J(x) + \frac{d}{dx}\Gamma(x(1-x)) = J(x) + \Gamma(1-2x)
    \]

    For a selective population this gives \(J_{\Gamma}(1) = J(1) - \Gamma\). The
proof follows in the same way as for
Theorem~\ref{theorem:selective}.  \end{proof}
...
\begin{figure}[!htbp]
    ...
    \caption{Evolutionary stable populations for varying values of \(t, r,
        \Gamma\) for 6 difference scenarios. Varying values (TODO Include them),
    scenarios 5 and 6 have \(\Gamma=4\) (high) and \(r\) close to 1.}
    \label{fig:convergence-over-r}
\end{figure}


\begin{itemize}
    \item Too high value of \(r\) will swap the population.
    \item \(Gamma\) is only thing that can actually be controlled.
    \item This confirms that devaluing alone is not a solution and in fact can
        potentially have averse consequences: combinations of dehorning and
