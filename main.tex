...
\begin{proof}
    ...
    \begin{eqnarray}
    \label{eqn:selective_rhs}
    u(0,s^*)&=&
    H(\theta_rr + 1 - r)\theta(r, s^*)^{-\alpha} - F(1 - rs^*)^{\gamma}(1 - r)^{\beta}.
    \end{eqnarray}
    Substituting (\ref{eqn:selective_lhs}) and (\ref{eqn:selective_lhs}) into~(\ref{eqn:s1xs_s0xs}) gives

    \begin{eqnarray}\label{eqn:stable_mixed}
    H\theta(r, s^*)^{-\alpha} (-\theta_r r)  + F (1 - rs^*)^{\gamma}(1 - r)^{\beta}(\frac{r}{1-r}) &=& 0.\\
...
Jonathan Gillard for his careful reading and double checking of the algebra in the paper.
