...
\todo[backgroundcolor=blue!25]{I don't understand how this expression is correct.}
...
\end{eqnarray}
\todo[backgroundcolor=blue!25]{There's an \(r\) term missing here: should be
\(\theta_rr\)}
...
\end{eqnarray}
\todo[backgroundcolor=blue!25]{This is correct.}
...
\todo[backgroundcolor=blue!25]{Technically the expression doesn't hold for
\(r=1\), this is due to the fact that \(\beta\geq \) so
\((1-r)^{-\beta}=\frac{1}{(1-r)^{\beta}}\). However \(\lim_{r\to 1}(17)=-\inf\),
because of the \(R_1>R_2\), so I think we can say that this is not stable in the
limit case of \(r\to1\).}
\begin{eqnarray}
\label{eqn:rcond1}
